...
In the image, there can be seen some counts with 0 degrees. This proves that there are questions which hasn't been answered in the network. There is an inverse proportional relationship between the degree and the count of that degree(The higher the degree, the lower the count). Its rare to see a question with many answers, this is normally due to a potential accepted answer, so there would be no need to answer a question that is already answered/solved by someone else.
