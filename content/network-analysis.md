...
In the image, there can be seen some counts with 0 degrees. This proves that there are questions which hasn't been answered in the network. There is an inverse proportional relationship between the degree and the count of that degree(The higher the degree, the lower the count). A question with many answers doesn't happen as frequently as a question with few answers. This is normally due to a potential accepted answer, so there would be no need to answer a question that is already answered/solved by someone else.
